...
3. supporting_evidence: which source IDs (e.g. src_0) support each pattern/topic.
   You MUST only use IDs from this exact list, copied verbatim: {valid_sources}.
   Never invent an ID that isn't in that list, and never cite an ID whose content
   doesn't actually support the pattern/topic you're attaching it to.
